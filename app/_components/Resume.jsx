...
import { Download, Eye, Calendar, MapPin, Mail, Phone, Github, Linkedin, Award, Briefcase, GraduationCap, Code, Trophy } from 'lucide-react'
...
export default function Resume() {
  ...
  const projects = [
    ...
    },
    {
      ...
      date: "June 2025",
      type: "Full-Stack Project",
      technologies: ["Next.js", "Prisma ORM", "Vonage Video API", "PostgreSQL (Neon)"],
  ...
  const skills = {
    "Languages & Tools": ["Python", "JavaScript", "C/C++", "SQL", "React", "Node.js", "Express", "Prisma", "PostgreSQL", "REST APIs"],
    "Others": ["Next.js", "Supabase", "Clerk", "Stripe", "Vonage API", "Inngest", "Arcjet", "QGIS", "TRAZER", "Data Analysis", "AI/OCR"]
  }
  ...
  const positions = [
    {
      ...
      description: "Organized cultural events, leading a team of 200+ volunteers",
      year: "2024"
    },
    {
      title: "Head",
  ...
  const achievements = [
    ...
    },
    {
      ...
      description: "For being in top 0.1 percent candidates of CBSE Board Exam 2017-18",
      year: "2018"
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-3xl mx-auto mb-8">
            A comprehensive overview of my education, experience, and achievements in technology and engineering.
          </p>
          ...
          >
            ...
            {activeSection === "overview" && (
              <Card className="dark:bg-gray-700 dark:border-gray-600">
                <CardHeader>
                  <CardTitle className="text-2xl dark:text-white">{personalInfo.name}</CardTitle>
                  <p className="text-lg text-gray-600 dark:text-gray-300">
                    {personalInfo.degree}, {personalInfo.institute}
                  </p>
                  <p className="text-sm text-blue-600 dark:text-blue-400">
                    CGPA : {personalInfo.CGPA} | Dept. Rank: {personalInfo.deptRank}
                  </p>
                </CardHeader>
                  ...
                  {education.map((edu, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h3 className="text-lg font-semibold dark:text-white">{edu.degree}</h3>
                          <p className="text-gray-600 dark:text-gray-300">{edu.institute}</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400">{edu.board}</p>
                          {edu.highlight && (
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-blue-600 dark:text-blue-400">{edu.score}</p>
                  ...
                  {workExperience.map((exp, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <h3 className="text-lg font-semibold dark:text-white">{exp.title}</h3>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          ...
                          <p className="text-green-600 dark:text-green-400">{project.type}</p>
                          <div className="flex flex-wrap gap-2 mt-2">
                            {project.technologies.map((tech) => (
                              <Badge key={tech} variant="outline" className="dark:border-gray-500">
                                {tech}
                              </Badge>
                            ))}
                          </div>
                      ...
                      <ul className="space-y-2">
                        {project.description.map((desc, descIndex) => (
                          <li key={descIndex} className="flex items-start text-gray-600 dark:text-gray-300 text-sm">
                ...
                <CardContent className="space-y-6">
                  {Object.entries(skills).map(([category, skillList], index) => (
                    <motion.div
                      ...
                      transition={{ delay: index * 0.1 }}
                    >
                      <h3 className="text-lg font-semibold dark:text-white mb-3">{category}</h3>
            ...
            {activeSection === "achievements" && (
              <Card className="dark:bg-gray-700 dark:border-gray-600">
                <CardHeader>
                  <CardTitle className="dark:text-white">Achievements & Positions</CardTitle>
                </CardHeader>
                <CardContent className="space-y-8">
                  <div>
                    <h3 className="text-lg font-semibold dark:text-white mb-4">Positions of Responsibility</h3>
                      ...
                      {positions.map((position, index) => (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-semibold dark:text-white">{position.title}</h4>
                              <p className="text-orange-600 dark:text-orange-400">{position.organization}</p>
                              {position.description && (
                                <p className="text-gray-600 dark:text-gray-300 text-sm mt-1">{position.description}</p>
                              )}
                            </div>
                            <span className="text-sm text-gray-500 dark:text-gray-400">{position.year}</span>
                          </div>
                        </motion.div>
                      ))}
                    </div>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold dark:text-white mb-4">Miscellaneous Achievements</h3>
                    <div className="space-y-4">
                      {achievements.map((achievement, index) => (
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-semibold dark:text-white">{achievement.title}</h4>
